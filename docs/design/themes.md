...
- **Runtime mode overrides**: player mode (`<html data-mode="player">`) collapses `--shell-offset` to 40px. Themes declare this override inside their own file (e.g. `:root[data-theme='tech'][data-mode='player']`) so layout changes flow through CSS only — no React branching
