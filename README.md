...
- **サッと見て、それで終わり** ── 通知やランキングで呼び戻したりはしません。テレビはタイムラインではないので。
- **外出先でも家と変わらず見られること** ── 細い回線でも止まらないよう、画質や映像形式の調整は裏側で自動的に行います。
- **仕組みを意識させないこと** ── チューナーが空いてるかとか、コーデックが何とか、そういう話はユーザーには関係ないはずなので。裏で勝手にうまくやっておきたいです。
- **いずれテレビの大画面でも使えること** ── Apple TV / Fire TV でリモコン操作できるような画面構成を、今のうちから意識しておきます。
