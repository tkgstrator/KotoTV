 ...
 */
export async function acquireLive(req: LiveSessionRequest): Promise<{ sessionId: string; playlistUrl: string }> {
  ...
  try {
    await waitForPlaylist(`${outputDir}/playlist.m3u8`, 10_000)
  } catch (err) {
    proc.kill()
  ...
  // Self-heal if FFmpeg dies (driver crash, etc.) — drop the registry entry so
